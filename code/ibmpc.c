...
void ibmeeol(void) {        /* erase to the end of the line */
...
#endif
    lnptr = &sline[0];
    for (i = 0; i < term.t_ncol; i++) *lnptr++ = SPACE | attr;
...
void ibmputc(int ch) {
    ...
    rg.h.al = ch;
#if COLOR
    if (dtype != CDMONO) rg.h.bl = cfcolor;
    else                 rg.h.bl = 0x07;
#else
    rg.h.bl = 0x07;
#endif
...
/* If we had the EGA open... close it */
    if (dtype == CDEGA) egaclose();
    if (dtype == CDVGA) egaclose();
    ttclose();
}
...
#endif
    lnptr = &sline[0];
    for (i = 0; i < term.t_ncol; i++) *lnptr++ = (outstr[i] & 255) | attr;
