 ...
 *
 * uemacs now uses dyn_buf variables, so these constraints have gone.
 */
#ifndef PATH_MAX
...
struct func_opts {
    ...
    unsigned int no_macbug :1;      /* No macbug display while running */
};
...
struct region {
    ...
    ue64I_t r_bytes;            /* Length in bytes.             */
...
enum ev_val {
    ...
    EVCBUFNAME, EVCFNAME,   EVDFNAME,   EVDEBUG,        EVSTATUS,
    EVASAVE,    EVACOUNT,   EVLASTKEY,  EVCURCHAR,      EVDISCMD,
    EVVERSION,  EVPROGNAME, EVSEED,     EVDISINP,       EVWLINE,
    EVCWLINE,   EVTARGET,   EVSEARCH,   EVREPLACE,      EVMATCH,
    EVKILL,     EVCMODE,    EVGMODE,    EVPENDING,      EVLWIDTH,
